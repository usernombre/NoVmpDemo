...
struct opcode_descriptor {
    ...
    bool reduce(vm_state *vstate, instruction &ins, opcode_id identifier,
                const std::vector<char> &variants = {}) {
        // If parameter _count_ does not match, return failure right away
        //
        if (ins.parameter_sizes.size() != parameter_sizes.size())
            return false;
        ...
        for (int i = 0; i < parameter_sizes.size(); i++) {
            ...
            // return failure
            //
            if (parameter_sizes[i] != ins.parameter_sizes[i] && parameter_sizes[i] != PANY)
                return false;
        }
        ...
        //
        if (!matcher(vstate, ins, variants))
            return false;
...
//
static bool i_write_vsp(vm_state *vstate, const vtil::amd64::instruction &i, uint32_t offset,
                        char variant) {
    return (variant == 1 ? i.id == X86_INS_MOV || i.id == X86_INS_MOVZX : i.id == X86_INS_MOV) &&
           i.operands[0].type == X86_OP_MEM && i.operands[0].mem.base == vstate->reg_vsp &&
           i.operands[0].mem.index == X86_REG_INVALID && i.operands[0].mem.disp == offset;
}

static bool i_read_vsp(vm_state *vstate, const vtil::amd64::instruction &i, uint32_t offset,
                       char variant) {
    return (variant == 1 ? i.id == X86_INS_MOV || i.id == X86_INS_MOVZX : i.id == X86_INS_MOV) &&
           i.operands[1].type == X86_OP_MEM && i.operands[1].mem.base == vstate->reg_vsp &&
           i.operands[1].mem.index == X86_REG_INVALID && i.operands[1].mem.disp == offset;
}

static bool i_ref_vsp(vm_state *vstate, const vtil::amd64::instruction &i, uint32_t offset = 0) {
    if (!offset && i.id == X86_INS_MOV && i.operands[1].type == X86_OP_REG &&
        i.operands[1].reg == vstate->reg_vsp) {
        return true;
    }

    return i.id == X86_INS_LEA && i.operands[1].type == X86_OP_MEM &&
...
static bool i_shift_vsp(vm_state *vstate, const vtil::amd64::instruction &i, int32_t offset) {
    ...
    if (offset > 0) {
        return i.id == X86_INS_ADD && i.operands[0].type == X86_OP_REG &&
               i.operands[0].reg == vstate->reg_vsp && i.operands[1].type == X86_OP_IMM &&
               i.operands[1].imm == offset;
    } else {
        return i.id == X86_INS_SUB && i.operands[0].type == X86_OP_REG &&
               i.operands[0].reg == vstate->reg_vsp && i.operands[1].type == X86_OP_IMM &&
               i.operands[1].imm == -offset;
    }
}
...
static bool i_write_ctx(vm_state *vstate, const vtil::amd64::instruction &i, char variant,
                        int32_t disp = 0) {
    return (variant == 1 ? i.id == X86_INS_MOV || i.id == X86_INS_MOVZX : i.id == X86_INS_MOV) &&
           i.operands[0].type == X86_OP_MEM && i.operands[0].mem.base == X86_REG_RSP &&
           i.operands[0].mem.index != X86_REG_INVALID && i.operands[0].mem.scale == 1 &&
           i.operands[0].mem.disp == disp &&
           (variant == 1 ? i.operands[1].size <= 2 : i.operands[1].size == variant);
}

static bool i_read_ctx(vm_state *vstate, const vtil::amd64::instruction &i, char variant,
                       int32_t disp = 0) {
    return (variant == 1 ? i.id == X86_INS_MOV || i.id == X86_INS_MOVZX : i.id == X86_INS_MOV) &&
           i.operands[1].type == X86_OP_MEM && i.operands[1].mem.base == X86_REG_RSP &&
           i.operands[1].mem.index != X86_REG_INVALID && i.operands[1].mem.scale == 1 &&
           i.operands[1].mem.disp == disp &&
           (variant == 1 ? i.operands[1].size <= 2 : i.operands[1].size == variant);
}

static bool i_save_vsp_flags(vm_state *vstate, const vtil::amd64::instruction &i0,
                             const vtil::amd64::instruction &i1, uint32_t offset = 0) {
    return i0.id == X86_INS_PUSHFQ && i1.id == X86_INS_POP && i1.operands[0].type == X86_OP_MEM &&
           i1.operands[0].mem.base == vstate->reg_vsp &&
           i1.operands[0].mem.index == X86_REG_INVALID && i1.operands[0].mem.disp == offset;
}
...
std::map<opcode_id, opcode_descriptor> opcodes = {
    ...
    */
    ...
    /*
            # [Adds two integers from user stack and overwrites them with the results of the
       operation]
            #
            # Pseudocode:
            # ----------------------------------------------
    ...
    */
    ...
    /*
            # [NAND two integers from user stack and overwrites them with the results of the
       operation]
            #
            # Pseudocode:
            # ----------------------------------------------
    ...
    */
    ...
    /*
            # [NOR two integers from user stack and overwrites them with the results of the
       operation]
            #
            # Pseudocode:
            # ----------------------------------------------
    ...
    */
    ...
    /*
            ...
            #	t0			:=	[VSP]
            #	t1			:=	[VSP+*]
            #	t2			:=	[VSP+2*]b (but kinda treated like WORD because yea
       whatever LOL) #	VSP			+=  (*-6) #	SHRD(t0, t1, t2) #	[VSP+8]
       :=	t0 #	[VSP]		:=	tf # ----------------------------------------------
            #
            # VSHRDU*()
    */
    ...
    /*
            ...
            #	t1			:=	[VSP+*]
            #	t2			:=	[VSP+2*]b (but kinda treated like WORD because yea
       whatever LOL) #	VSP			+=  (*-6) #	SHLD(t0, t1, t2) #	[VSP+8]
       :=	t0 #	[VSP]		:=	tf # ----------------------------------------------
            #
            # VSHLDU*()
    */
    ...
    */
     ...
     {{},
      ...
      [](vm_state *vstate, instruction &vins, const std::vector<char> &var) -> bool {
          ...
          return is.size() == (3 + dt) &&
                 ...
                 is[1].operands[1].mem.disp == 0 && is[1].operands[1].size == var[0] &&
    ...
    /*
            # [Pops a pointer from user stack, pops a u* from the stack and writes it into the
       pointer]
            #
            # Pseudocode:
            # ----------------------------------------------
     ...
     {{},
      ...
      [](vm_state *vstate, instruction &vins, const std::vector<char> &var) -> bool {
          ...
          return is.size() == 4 &&
                 ...
                 is[3].id == X86_INS_MOV && is[3].operands[0].type == X86_OP_MEM &&
                 is[3].operands[0].mem.index == X86_REG_INVALID && is[3].operands[0].mem.disp == 0;
      }}},

    /*
            # [LOCK XCHG the pointer on top of the stack with the value on top of that, discard the
       pointer]
            #
            # Pseudocode:
            # ----------------------------------------------
    ...
    */
    ...
    /*
            ...
            # ----------------------------------------------
            #   t0			:=  [VSP]
            #   VSP			=	t0		# Not visible in instruction stream
       beacuse it's below JA # ----------------------------------------------
            #
            # VSETVSP()
    */
    {"VSETVSP",
     {{},
      ...
      [](vm_state *vstate, instruction &vins, const std::vector<char> &var) -> bool {
          ...
          return is.size() == 1 &&
                 ...
                 i_read_vsp(vstate, is[0], 0, 8) && is[0].operands[0].type == X86_OP_REG &&
                 // CHANGED
      ...
      [](vm_state *vstate, instruction &vins, const std::vector<char> &var) -> bool {
          ...
          if (is.size() == 2) {
              return
                  ...
                  is[0].id == X86_INS_ADD && is[0].operands[0].type == X86_OP_REG &&
                  // is[ 0 ].operands[ 0 ].reg == vstate->reg_vsp && (Can't check this as this is
                  // pre-mutation VSP)
                  is[0].operands[1].type == X86_OP_IMM && is[0].operands[1].imm == 8 &&

                  // [[ 000000014009FCEC: lea  rbx, [rip - 7]
                  is[1].id == X86_INS_LEA;
          } else if (is.size() == 3) {
              // Out of pure coincidence VIP regsiter matches, so another instruction is appended
              // LUL
              return is[0].id == X86_INS_MOV &&

                     // [[ 00000001400E6301: add  rbp, 8
                     is[1].id == X86_INS_ADD && is[1].operands[0].type == X86_OP_REG &&
                     // is[ 1 ].operands[ 0 ].reg == vstate->reg_vsp && (Can't check this as this is
                     // pre-mutation VSP)
                     is[1].operands[1].type == X86_OP_IMM && is[1].operands[1].imm == 8 &&
     ...
     {{},
      ...
      [](vm_state *vstate, instruction &vins, const std::vector<char> &var) -> bool {
          ...
          return is.size() == 3 &&
                 ...
                 is[0].id == X86_INS_MOV && is[0].operands[1].reg == X86_REG_CR0 &&
     ...
     {{},
      ...
      [](vm_state *vstate, instruction &vins, const std::vector<char> &var) -> bool {
          ...
          return is.size() == 3 &&
                 ...
                 // [[ 0000000140095243: mov r9, cr0
                 is[0].id == X86_INS_MOV && is[0].operands[1].reg == X86_REG_CR3 &&
...
instruction classify(vm_state *vstate, const instruction_stream &is) {
    ...
    auto stack_instructions = {
        X86_INS_MOV, X86_INS_MOVZX, X86_INS_MOVSX, X86_INS_ADD,
        X86_INS_SUB, X86_INS_XOR,   X86_INS_OR,    X86_INS_AND,
    };
    for (int i = 0; i < is.size(); i++) {
        ...
        if (!stack_op_rtarget && !stack_op_wtarget) {
            ...
            auto process_mem_target = [&](const x86_op_mem *mem) {
                return mem->index == X86_REG_INVALID ? (out.stack_delta + mem->disp)
                                                     : unknown_delta;
            };
        ...
        // -- inc vsp
        else if (ins.is(X86_INS_INC, {X86_OP_REG}) && ins.operands[0].reg == vstate->reg_vsp) {
            out.stack_delta++;
        }
        // -- dec vsp
        else if (ins.is(X86_INS_DEC, {X86_OP_REG}) && ins.operands[0].reg == vstate->reg_vsp) {
            out.stack_delta--;
        }
        ...
        auto is_ctx_mem_op = [](const cs_x86_op &op) -> std::optional<x86_reg> {
            if (op.type == X86_OP_MEM && op.mem.base == X86_REG_RSP &&
                op.mem.index != X86_REG_INVALID && op.mem.scale == 1 && op.mem.disp == 0) {
                return op.mem.index;
            }
            return {};
        };
