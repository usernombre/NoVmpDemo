...
struct instruction_stream {
    ...
    std::vector<uint8_t> to_raw() const { return instruction_stream(*this).to_raw(); }
    ...
    std::pair<instruction_stream, std::vector<x86_reg>> trace(x86_reg ireg, int end,
                                                              int begin = 0) const {
        ...
        for (int i = end; i >= begin; i--) {
            ...
            for (int j = 0; j < ins.operands.size(); j++) {
                if (ins.operands[j].type == X86_OP_REG) {
                    ...
                    for (auto reg : {ins.operands[j].mem.base, ins.operands[j].mem.index}) {
                        if (vtil::amd64::registers.extend(reg) !=
                            vtil::amd64::registers.extend(ireg)) {
                            access_list.push_back(reg);
                            continue;
                        }
                        read |= ins.operands[j].access & CS_AC_READ;
    ...
    //
    int next(uint32_t instruction_id, const std::vector<x86_op_type> &operands,
             int from = 0) const {
        for (int i = from; i < stream.size(); i++)
            if (stream[i].second.is(instruction_id, operands))
                return i;
        return -1;
    }

    int next(const fn_instruction_filter &filter, int from = 0) const {
        for (int i = from; i < stream.size(); i++)
            if (filter(stream[i].second))
                return i;
        return -1;
    }

    int next(uint32_t instruction_id, const std::vector<x86_op_type> &operands,
             const fn_instruction_filter &filter, int from = 0) const {
        for (int i = from; i < stream.size(); i++)
            if (stream[i].second.is(instruction_id, operands) && filter(stream[i].second))
                return i;
        return -1;
    }

    int prev(uint32_t instruction_id, const std::vector<x86_op_type> &operands,
             int from = -1) const {
        if (from == -1)
            from = stream.size() - 1;
    ...
    int prev(const fn_instruction_filter &filter, int from = -1) const {
        if (from == -1)
            from = stream.size() - 1;
    ...
    int prev(uint32_t instruction_id, const std::vector<x86_op_type> &operands,
             const fn_instruction_filter &filter, int from = -1) const {
        if (from == -1)
            from = stream.size() - 1;
    ...
    // Convert the instruction sequence to a string
    //
    ...
    // Dump the given instruction sequence to stdout
    //
    void dump() {
        for (auto &p : stream)
            log<CON_GRN>("%s\n", p.second.to_string());
...
static instruction_stream deobfuscate(image_desc *img, uint32_t rva_rip) {
    ...
    if (output.stream.empty())
        vtil::logger::error("Failed to unroll control-flow.");
    return output;
}
}; // namespace vmp